...
async def test(dut):
    ...
    Example AXI-STREAM5 interface
        ...
        - tuser
        - twakeup
        - 100 items in the request sequence with a rate limit of 0.1, keep rate of 0.90 and sleep rate of 0.25
        - random data
    ...
    avl.Factory.set_variable("*.agent.tsqr.tseq.packet_length", lambda: random.choice([32, 40, 48, 56]))
    avl.Factory.set_variable("*.agent.tsqr.tseq.keep_rate", lambda: 0.90)
    avl.Factory.set_variable("*.agent.tsqr.tseq.sleep_rate", lambda: 0.25)
